	...
	private static String managerInterface(){
		...
		try{
			...
			case("registerRestaurant"):
				...
				if(!error){
					currentManager.addUser(UserType.restaurant, restaurantName, "", restaurantUserName, restaurantPassword);
					try{
						((Restaurant)currentManager.getMyFoodora().findUserByName(restaurantName)).setAddress(new Location(restaurantX,restaurantY));
			...
			case("registerCourier"):
				...
				if(!error){
					currentManager.addUser(UserType.courier, courierName, courierSurname, courierUserName, courierPassword);
					try{
						((Courier)currentManager.getMyFoodora().findUserByName(courierName)).setPosition(new Location(courierX,courierY));
	...
	private static String restaurantInterface(){
		...
		try{
			...
			case("addDishRestaurantMenu"):
				...
				DishType dishType = DishType.starter;
				switch(dishCategory){
				case("starter"):
				...
				DietType diet = DietType.glutenFree;
				switch(foodCategory){
				case("standard"):
				...
				if(!error){
					currentRestaurant.addDish(dishType, dishName, price, diet);
